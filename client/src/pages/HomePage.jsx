...
const HomePage = () => {
  ...
  // Calculate related products for "You may also like"
  const filteredProductIds = new Set(filteredProducts.map(p => p._id));

  const relatedProductsSet = new Map();

  if (filteredProducts.length > 0 && productData) {
    filteredProducts.forEach(fp => {
      productData.forEach(p => {
        if (
          p.category === fp.category &&
          !filteredProductIds.has(p._id) &&
          !relatedProductsSet.has(p._id)
        ) {
          relatedProductsSet.set(p._id, p);
        }
      });
    });
  }

  const relatedProducts = Array.from(relatedProductsSet.values());
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white py-16">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center">
            ...
            {/* SEARCH RESULTS: Show here if search is not empty */}
            {search && (
